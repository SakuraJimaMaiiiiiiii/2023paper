function c = Fast_Kurtogram_1(x,nlevel,Fs,opt1,opt2)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [Kf,M4,M2,k] = Kf_W2(x,Nfft,Noverlap,Window,opt)
...
figure,plot(tf,real(xf));
figure,plot(f,S(1:nfft/2));
...
if spec == 1
   ...
   subplot(313),plot(f,S(1:nfft/2),'color',0.5*[1 1 1]),title('Fourier transform magnitude of the squared envelope'),xlabel('frequency [Hz]')
   
end
f = linspace(0,.5*Fs/2^level,nfft/2);
figure,plot(f,S(1:nfft/2),'color',0.5*[1 1 1]);
figure,plot(tc,real(c),'color',0.5*[1 1 1]);
